...
FractionToPercentageConverter::FractionToPercentageConverter(NumberFormat config) : _config(config){}

std::string FractionToPercentageConverter::convert(const Fraction& f)
{
	Fraction temp = f;
	std::string result = std::to_string(100 * (double)temp.getNumerator() / temp.getDenominator());
